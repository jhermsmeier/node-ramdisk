...
function RamDisk( options ) {
  ...
  if( this.size > RamDisk.INT_MAX )
    throw new Error( 'Disk size may not exceed MAX_SAFE_INTEGER bytes' )
  
  if( this.clusterCount > RamDisk.INT_MAX )
    throw new Error( 'Cluster count exceeds MAX_SAFE_INTEGER' )
 ...
 * @type {Number}
 */
RamDisk.INT_MAX = Number.MAX_SAFE_INTEGER ||
  Math.pow( 2, 53 ) - 1
